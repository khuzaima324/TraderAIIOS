...
export const Analyzer: React.FC<AnalyzerProps> = ({
  ...
  const pickImage = async (useCamera: boolean) => {
    ...
    try {
      const hasConsented = await AsyncStorage.getItem("ai_privacy_consent");

      if (!hasConsented) {
        Alert.alert(
          "AI Analysis Consent",
          "By continuing, you agree to our Privacy Policy and Terms of Use. Your images will be processed by Google AI services to provide analysis.",
          [
            {
              text: "View Privacy Policy",
              style: "cancel",
              onPress: () => {
                Linking.openURL("https://traders-ai-privacy-policy.vercel.app");
              },
            },
            {
              text: "Agree & Continue",
              style: "default",
              onPress: async () => {
                await AsyncStorage.setItem("ai_privacy_consent", "true");
                executePickImage(useCamera); // 👈 This opens the gallery/camera after they agree
              },
            },
          ]
        );
      ...
      // If already consented, proceed directly
      executePickImage(useCamera);
    } catch (e) {
      console.error("Consent check error", e);
    }
  };

  const executePickImage = async (useCamera: boolean) => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // Detect the failure sentinel returned after all retries are exhausted
      if (
        setup.asset === "__ANALYSIS_FAILED__" ||
        setup.symbol === "__ANALYSIS_FAILED__"
      ) {
        setErrorMsg(
          "Analysis failed after multiple attempts. Please check your connection and try again.",
        );
        setResult(null);
        setIsAnalyzing(false);
        return;
            ...
            <View style={styles.privacyDisclaimer}>
              ...
              <Text style={styles.privacyText}>
                ...
                <Text 
                  ...
                  onPress={() => Linking.openURL("https://traders-ai-privacy-policy.vercel.app")}
                >
                  Privacy Policy
                </Text>
              </Text>
        ...
        {result && (
          <View style={{ paddingBottom: 40 }}>
            <GlassCard style={styles.resultCard}>
              ...
              {/* Price Grid — hidden/greyed when NEUTRAL */}
              <View style={styles.resultGrid}>
                <View style={styles.gridItem}>
                  <Text style={styles.gridLabel}>{t.entry}</Text>
                  <Text
                  ...
                  >
                    {result.direction === "NEUTRAL" ? "—" : result.entryPrice}
                  </Text>
                </View>
                <View
                  style={[
                ...
                >
                  ...
                  >
                    {result.direction === "NEUTRAL" ? "—" : result.stopLoss}
                  </Text>
                </View>
                <View style={styles.gridItem}>
                  <Text
                  ...
                  >
                    {result.direction === "NEUTRAL" ? "—" : result.takeProfit}
                  </Text>
                </View>
              </View>
              ...
              {result.direction === "NEUTRAL" && (
                <View style={styles.neutralBanner}>
                  <Text style={styles.neutralBannerText}>
                    ⚠️ Neutral signal — no trade levels available. Wait for a
                    clearer setup.
                  </Text>
                </View>
              )}
                ...
                {showAdvanced && (
                  <View style={styles.advancedBody}>
                    <View style={styles.structureBox}>
                      ...
                      {result.advanced?.marketStructure?.breakOfStructure &&
                        result.advanced.marketStructure.breakOfStructure !==
                          "None detected" && (
                      ...
                      {result.advanced?.marketStructure?.changeOfCharacter &&
                        result.advanced.marketStructure.changeOfCharacter !==
                          "None detected" && (
                          <Text style={styles.chochText}>
                            ⚡ CHoCH:{" "}
                            {result.advanced.marketStructure.changeOfCharacter}
                          </Text>
                        )}
                    ...
                    <View style={{ marginVertical: 10 }}>
                      ...
                      {(() => {
                        const allPatterns = [
                          ...
                          (p) =>
                            p !== "No significant candle patterns" &&
                            p !== "No significant chart patterns",
                        );
                    ...
                    {/* Volume & Momentum — shown for PLUS and PRO */}
                    {result.advanced?.volumeMomentum && (
                      <View style={styles.infoRow}>
                        <Text style={styles.subHeader}>Volume & Momentum</Text>
                    ...
                    {/* Indicators — shown for PLUS and PRO */}
                    {result.advanced?.indicators &&
                      result.advanced.indicators !==
                        "Indicators not visible on this chart" && (
                        <View style={styles.infoRow}>
                          <Text style={styles.subHeader}>Indicators</Text>
                          <Text style={styles.infoText}>
                            {result.advanced.indicators}
                          </Text>
                        </View>
                    ...
                    {userTier === "PRO" ? (
                      <>
                        ...
                        >
                          ...
                          >
                            <View style={styles.zoneBox}>
                              <Text style={styles.zoneLabel}>FVG</Text>
                              <Text style={styles.zoneValue}>
                                {result.advanced?.keyLevels?.fairValueGaps
                                  ?.filter(
                                    (v) =>
                                      v !== "Not visible on this timeframe",
                                  )
                                  .join(", ") || "Not visible"}
                              </Text>
                            </View>
                            <View style={styles.zoneBox}>
                              <Text style={styles.zoneLabel}>Order Blocks</Text>
                              <Text style={styles.zoneValue}>
                                {result.advanced?.keyLevels?.orderBlocks
                                  ?.filter(
                                    (v) =>
                                      v !== "Not visible on this timeframe",
                                  )
                          ...
                          >
                            <View style={styles.zoneBox}>
                              <Text style={styles.zoneLabel}>Supply Zones</Text>
                              <Text style={styles.zoneValue}>
                                {result.advanced?.keyLevels?.supplyZones
                                  ?.filter((v) => v !== "Not identified")
                                  .join(", ") || "Not identified"}
                              </Text>
                            </View>
                            <View style={styles.zoneBox}>
                              <Text style={styles.zoneLabel}>Demand Zones</Text>
                              <Text style={styles.zoneValue}>
                                {result.advanced?.keyLevels?.demandZones
                                  ?.filter((v) => v !== "Not identified")
                                  .join(", ") || "Not identified"}
                              </Text>
                            </View>
                        ...
                        {/* Liquidity Map */}
                        {(result.advanced?.liquidityMap?.sweepLevels ||
                          result.advanced?.liquidityMap?.magnetZones) && (
                          <View style={{ marginTop: 15 }}>
                            <Text style={styles.subHeader}>Liquidity Map</Text>
                            <View style={styles.liquidityBox}>
                              <Text style={styles.liquidityLabel}>
                                🎯 Sweep Levels
                              </Text>
                            ...
                            <View
                              style={[styles.liquidityBox, { marginTop: 6 }]}
                            >
                              <Text style={styles.liquidityLabel}>
                                🧲 Magnet Zones
                              </Text>
